...
app.get("/readData", (req, res) => {
    fs.readdir(__dirname + "/Data/newRequest", function (err, filenames) {
        if (err) {
            console.log(err);
            return;
        }
        ...
        filenames.forEach(function (filename) {
            try {
                userData = JSON.parse(fs.readFileSync(__dirname + "/Data/newRequest/" + filename, "utf8"));
            } catch (error) {
                console.log(error);
            }

            if (userData !== undefined) {
                responseData[filename] = userData;
                userData = undefined;
            }
            fs.rename(__dirname + "/Data/newRequest/" + filename, __dirname + "/Data/oldRequests/" + filename, function (err) {
                if (err) {
                    console.log("couldn't able to move " + filename);
                }
                else {
...
app.post("/sendChecklistData", (req, res) => {
    ...
    if (req.body.result.metadata.intentId === "42017ae9-a1b8-4a20-b67d-783835238a3b") {
        var data2store = {
            ...
            emailId: req.body.originalRequest.data.data.personEmail,
            checklistType: "PET"
        }
